...
class YahooHistoryDownloader(Downloader, Producer, pages={"history": YahooHistoryPage}):
    def execute(self, *args, tickers=[], dates, **kwargs):
        for ticker in tickers:
            history = self.pages["history"](ticker, *args, dates=dates, **kwargs)
            history = history.reset_index(drop=True, inplace=False)
            yield dict(ticker=ticker, historicals=history)
